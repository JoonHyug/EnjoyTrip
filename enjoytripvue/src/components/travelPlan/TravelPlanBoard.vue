<template>
  <div class="my-10 overflow-hidden flex items-center justify-center">
    ...
    <div class="w-[80%]">
      ...
      <div
        ...
        <div class="flex justify-between">
          <div class="items-center flex min-w-[8rem] justify-center">
            <p class="text-lg text-center font-bold text-gray-600">여행 경로</p>
          </div>
          <div
            ...
            <select
              ...
              v-model="key">
              <option value="createTime" selected readonly>선택</option>
      ...
      <!-- 글들 -->
      <div
        class="align-middle inline-block min-w-full shadow overflow-hidden bg-white shadow-dashboard px-8 pt-3 pb-6 border-y-2 border-gray-300">
        <table class="w-full">
          <thead>
            <tr>
              <th
                class="px-6 py-3 border-b-2 border-gray-300 text-center leading-4 text-blue-500 tracking-wider">
                <span class="cursor-pointer" @click="selectOrder('title')">
                  제목
                </span>
                <span>
                  <i
                    ...
                    :class="{
                      hidden: sort && order === 'title',
                      'text-blue-500': order === 'title',
                    }"></i>
                  <i
              ...
              <th
                class="px-6 py-3 border-b-2 border-gray-300 text-center text-sm leading-4 text-blue-500 tracking-wider">
                <span class="cursor-pointer" @click="selectOrder('createTime')">
                  작성일
                </span>
                <span>
                  <i
                    class="bi bi-arrow-down-circle text-blue-200"
                    :class="{
                      hidden: sort && order === 'createTime',
                      'text-blue-500': order === 'createTime',
                    }"></i>
                  <i
                    class="bi bi-arrow-up-circle text-blue-200"
                    :class="{
          ...
          <tbody class="bg-white">
            <tr v-for="item in items" :key="item.id">
              <td
                ...
                @click="goPath(`travelplan/${item.id}`)">
                <div
...
export default {
  ...
  data() {
    return {
      ...
      order: "createTime",
      showModal: false,
      confirmMessage: "계획의 제목을 입력해주세요",
      type: "input",
    };
  },
  computed: {
    ...mapState("travelplan", { items: "travelPlanList" }),
    ...mapState("travelplan", ["totalItems"]),
    ...mapState("travelplan", ["travelPlan"]),
  ...
  methods: {
    ...
    search() {
      this.$store.dispatch("travelplan/getTravelPlanList", {
      ...
      this.$store.dispatch("travelplan/getTotalItems", {
        word: this.word,
        key: this.key,
      });
    },
    ...
    async modalTitle(title) {
      console.log(title);
      await this.$store.dispatch("travelplan/addTravelPlan", {
        accountId: this.userInfo.id,
        title: title,
      });
      this.goPath("travelplan");
    },
  },
  created() {
    this.search();
